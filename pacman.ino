...
void intro() {
  lcd.clear(); lcd.setCursor(3, 0); lcd.print("VITAJ"); lcd.setCursor(1, 1); lcd.print("V PAC-MANOVI!");
  waitButton();
  lcd.clear(); lcd.setCursor(2, 0); lcd.print("JE TO JEDNODUCHE!");
  waitButton();
  lcd.clear(); lcd.setCursor(0, 0); lcd.print("Stlac tlacidlo"); lcd.setCursor(1, 1); lcd.print("aby si sa pohol");
  waitButton();
  lcd.clear(); lcd.setCursor(6, 0); lcd.print("ALE!");
  lcd.setCursor(0, 1); lcd.print("Nezabudni");
  waitButton();
  lcd.clear(); lcd.setCursor(3, 0); lcd.print("Zbieraj: "); lcd.write(byte(SPRITE_HEART)); lcd.setCursor(4, 1); lcd.print("Vyhybaj sa: "); lcd.write(byte(SPRITE_GHOST));
  waitButton(); lcd.clear(); lcd.setCursor(1, 0); lcd.print("Si pripraveny?"); lcd.setCursor(0, 1); lcd.print("Stlac tladidlo");
  waitButton();
  animation(1);
  state = STATE_PLAY;
}
...
void gameover() {
  animation(0);
  lcd.setCursor(3, 0); lcd.print("Koniec hry"); lcd.setCursor(0, 1); 
  waitButton();
  lcd.clear(); lcd.setCursor(2, 0); lcd.print("TVOJE SKORE:"); lcd.setCursor(7, 1); lcd.print(score);
  waitButton();
  if (score > highScore) {
    lcd.clear(); lcd.setCursor(1, 0); lcd.print("NOVE SKORE!");
    EEPROM.write(HIGHSCORE_ADDRESS, score);
    highScore = score;
    waitButton();
  }
  lcd.clear(); lcd.setCursor(3, 0); lcd.print("SKUS TO ZNOVA"); lcd.setCursor(2, 1); lcd.print("Najvysie skore: "); lcd.print(highScore);
  waitButton();
  initVars();
  state = STATE_PLAY;
  lcd.clear();
